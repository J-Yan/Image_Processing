...
def multi_Saab_transform(images, labels, kernel_sizes, num_kernels, energy_percent, use_num_images, use_classes):
    ...
    for i in range(num_layers):
        ...
        sample_patches = window_process2(sample_images, kernel_sizes[i], kernel_sizes[i])  # overlapping
        ...
        kernels, mean, pcaa = find_kernels_pca(training_data, num_kernel, energy_percent)
        pca.append(pcaa)
        # Add DC kernel
    ...
    return pca_params, pca


# Initialize
def initialize(sample_images, pca_params, pca):
    ...
    for i in range(num_layers):
        ...
        if i==0:
            ...
            sample_patches=window_process2(sample_images, kernel_sizes[i], kernel_sizes[i])
            print ('sample_patches1: {}'.format(sample_patches.shape))
