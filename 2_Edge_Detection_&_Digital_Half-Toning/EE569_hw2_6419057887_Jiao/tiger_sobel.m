...
I = cat(3,R,G,B);
Tig = flip(imrotate(I, -90),2);
imagesc(Tig);
fclose(fin);
% Tig_gray = rgb2gray(Tig);
% use own code to convert RGB to gray
Tig_gray = zeros(321,481);
for i = 1:321
    for j = 1:481
        Tig_gray(i,j) = (Tig(i,j,1)*0.2126 + Tig(i,j,2)*0.7152 + Tig(i,j,3)*0.0722);
    end
end
% sobel mask dir-x
...
for i = 2:320
    for j = 2:480
        patch = double([Tig_gray(i-1,j-1) Tig_gray(i-1,j) Tig_gray(i-1,j+1);
            Tig_gray(i,j-1) Tig_gray(i,j) Tig_gray(i,j+1);
            Tig_gray(i+1,j-1) Tig_gray(i+1,j) Tig_gray(i+1,j+1)]);
